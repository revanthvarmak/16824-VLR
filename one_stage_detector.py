...
class DetectorBackboneWithFPN(nn.Module):
    ...
    def forward(self, images: torch.Tensor):
        ...
        backbone_feats['c4'] = backbone_feats['c4'] + F.interpolate(backbone_feats['c5'], scale_factor=int(strides['p5']/strides['p4']))
        fpn_feats['p4'] = self.fpn_params['Conv_P4'](backbone_feats['c4'])
        backbone_feats['c3'] = self.fpn_params['Conv_C3'](backbone_feats['c3'])
        backbone_feats['c3'] = backbone_feats['c3'] + F.interpolate(backbone_feats['c4'], scale_factor=int(strides['p4']/strides['p3']))
...
class FCOSPredictionNetwork(nn.Module):
    ...
    def __init__(
        ...
        for k in range(num_layers):
            if k == 0:
                stem_cls.append(nn.Conv2d(in_channels, stem_channels[k], 3, 1, 1))
                nn.init.normal_(stem_cls[-1].weight, mean=0, std=0.01)
                nn.init.zeros_(stem_cls[-1].bias)
            else:
                stem_cls.append(nn.Conv2d(stem_channels[k-1], stem_channels[k], 3, 1, 1))
                nn.init.normal_(stem_cls[-1].weight, mean=0, std=0.01)
                nn.init.zeros_(stem_cls[-1].bias)
        ...
        for j in range(num_layers):
            if j == 0:
                stem_box.append(nn.Conv2d(in_channels, stem_channels[j], 3, 1, 1))
                nn.init.normal_(stem_box[-1].weight, mean=0, std=0.01)
                nn.init.zeros_(stem_box[-1].bias)
            else:
                stem_box.append(nn.Conv2d(stem_channels[j-1], stem_channels[j], 3, 1, 1))
                nn.init.normal_(stem_box[-1].weight, mean=0, std=0.01)
                nn.init.zeros_(stem_box[-1].bias)
    ...
    def forward(self, feats_per_fpn_level: TensorDict) -> List[TensorDict]:
        ...
        for lvl_name, fmap in feats_per_fpn_level.items():
            class_logits[lvl_name] = self.pred_cls(self.stem_cls(fmap))
            B, C, H, W = class_logits[lvl_name].shape

            class_logits[lvl_name] = torch.permute(class_logits[lvl_name].reshape(B, C, H*W), (0, 2, 1))
            boxreg_deltas[lvl_name] = self.pred_box(self.stem_box(fmap))

            B, C, H, W = boxreg_deltas[lvl_name].shape
            boxreg_deltas[lvl_name] = torch.permute(boxreg_deltas[lvl_name].reshape(B, C, H*W), (0, 2, 1))
            centerness_logits[lvl_name] = self.pred_ctr(self.stem_box(fmap))
            
            B, C, H, W = centerness_logits[lvl_name].shape
            centerness_logits[lvl_name] = torch.permute(centerness_logits[lvl_name].reshape(B, C, H*W), (0, 2, 1))
        ######################################################################
        #                           END OF YOUR CODE                         #
        ######################################################################
...
class FCOS(nn.Module):
    ...
    def forward(
        ...
        for j in range(images.shape[0]):
          matched_gt_boxes.append(fcos_match_locations_to_gt(locations_per_fpn_level,strides_per_fpn_level, gt_boxes[j]))
        ...
        for matched_gt_box_per_image in matched_gt_boxes:
          ...
          for lvl_name, gtbox in matched_gt_box_per_image.items():
            matched_gt_deltas[-1][lvl_name] = fcos_get_deltas_from_locations(locations_per_fpn_level[lvl_name], gtbox, strides_per_fpn_level[lvl_name])
        ######################################################################
        #                           END OF YOUR CODE                         #
        ######################################################################
