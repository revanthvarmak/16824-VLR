...
def fcos_make_centerness_targets(deltas: torch.Tensor):
    ...
    ##########################################################################
    # centerness = None
    num = torch.min(deltas[:, 0:4:2], dim=1).values * torch.min(deltas[:, 1:4:2], dim=1).values
    den = torch.max(deltas[:, 0:4:2], dim=1).values * torch.max(deltas[:, 1:4:2], dim=1).values
...
def get_fpn_location_coords(
    ...
    for level_name, feat_shape in shape_per_fpn_level.items():
        ...
        Ht = torch.arange(H, device=device, dtype=dtype)[:, None].repeat(1, W)
        Wt = torch.arange(W, device=device, dtype=dtype)[None, :].repeat(H, 1)
        Ht = level_stride * (Ht + 0.5).reshape(-1, 1)
        Wt = level_stride * (Wt + 0.5).reshape(-1, 1)
        location_coords[level_name] = torch.cat((Ht, Wt), dim=1)
        ######################################################################
        #                             END OF YOUR CODE                       #
        ######################################################################
